...
.DESCRIPTION
   Downloads and installs HPE SSA CLI
...
try{
    ...
    Invoke-WebRequest -Uri "https://downloads.hpe.com/pub/softlib2/software1/sc-windows/p955544928/v183348/cp044527.exe" -UseBasicParsing -OutFile "C:\Windows\Temp\cp044527.exe"
}
catch {
...
if (Test-Path "C:\Windows\Temp\cp044527.exe") {
    
    Write-Host "Info - File Downloaded. Will attempt to install..."
        
    try {
        Write-Host "Installing..."
        Start-Process -NoNewWindow -FilePath "C:\Windows\Temp\cp044527.exe" -ArgumentList '/s' -Wait
