...
$IMDBPath = "C:\Windows\temp\Imagemanager.mdb"
copy "C:\Program Files (x86)\StorageCraft\ImageManager\ImageManager.mdb" "C:\Windows\Temp\Imagemanager.mdb"
...
{ 
    ...
    if (Test-Path C:\Windows\Temp\AccessDatabaseEngine_X64.exe) {
    
        Write-Host "Info - File Downloaded. Will attempt to install..."
        
        try {
            Start-Process -NoNewWindow -FilePath "C:\Windows\Temp\AccessDatabaseEngine_X64.exe" -ArgumentList '/q' -Wait
        }
        catch {
...
Del "C:\Windows\Temp\Imagemanager.mdb"
